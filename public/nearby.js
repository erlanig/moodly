/* ═══════════════════════════════════════
   MOODLY — nearby.js  (v3 — no nominatim)
...
export async function findNearby() {
  ...
  setLoading(el, 'Mendeteksi lokasi kamu…');

  try {
    // Step 1: GPS
    const coords = await getLocation();

    setLoading(el, 'Mencari layanan kesehatan mental terdekat…');

    // Step 2: Langsung ke Claude (tanpa nominatim — hindari CORS)
    const results = await callClaude(coords);

    nb.results   = results.items;
    nb.cityName  = results.city;
    ...
    nb.loading = false;
    console.warn('[nearby] err:', err.message);
...
function setLoading(el, txt) {
  el.innerHTML = `
    <div class="nb-loading">
      ...
      <div class="nb-loading-txt">${txt}</div>
    </div>`;
}
...
/* ════════════════  CLAUDE API  ════════════════ */
async function callClaude(coords) {
  const { lat, lng } = coords;
  const today = new Date().toLocaleDateString('id-ID');

  const prompt = `Kamu asisten yang membantu menemukan layanan kesehatan mental di Indonesia.
Koordinat GPS pengguna: lat=${lat.toFixed(4)}, lng=${lng.toFixed(4)}.
Tanggal: ${today}.

Langkah 1: Gunakan web_search dengan query "layanan kesehatan mental psikolog terdekat lat ${lat.toFixed(2)} lng ${lng.toFixed(2)}" dan juga "psikolog [nama kota/area terdekat dari koordinat tersebut]" untuk mengetahui kota terdekat dan mencari layanan nyata di sana.

Langkah 2: Cari juga "konseling online Indonesia terpercaya 2024 2025" untuk layanan online nasional.

Kembalikan JSON dengan format ini PERSIS, HANYA JSON, tidak ada teks lain:
{
  "city": "<nama kota/area dari koordinat GPS>",
  "items": [
    {
      "id": 1,
      "type": "psikolog",
      ...
      "priceRange": "Rp 200.000-350.000/sesi",
      "tags": ["Anxiety", "Depresi", "Stres"],
      "isOnline": false,
      "emoji": "🧠",
      "description": "Satu kalimat mengapa layanan ini direkomendasikan."
    }
  ]
}

Sertakan 8 layanan total: campuran psikolog lokal, psikiater/RS, klinik, dan WAJIB minimal 2 layanan online nasional (Riliv, Into The Light Indonesia, Yayasan Pulih, atau sejenisnya).
Untuk layanan online: isOnline=true, address="Layanan Online - Seluruh Indonesia".`;
  ...
  if (!res.ok) {
    ...
    console.warn('[nearby] HTTP', res.status, txt.slice(0,200));
    throw new Error('api_fail');
  }

  const data = await res.json();
  console.log('[nearby] response stop_reason:', data.stop_reason, '| blocks:', data.content?.length);

  const textBlocks = (data.content || []).filter(c => c.type === 'text');
  if (!textBlocks.length) throw new Error('no_text');

  const raw = textBlocks.map(c => c.text || '').join('');
  console.log('[nearby] raw text (first 300):', raw.slice(0, 300));

  // Cari objek JSON { "city":..., "items":... }
  const match = raw.match(/\{[\s\S]*"city"[\s\S]*"items"[\s\S]*\}/);
  if (!match) {
    console.warn('[nearby] no JSON object found. Full raw:', raw.slice(0, 500));
  ...
  return `<div class="nb-card">
    ...
    <div class="nb-card-actions">
      ...
      ${r.website ? `<a class="nb-btn nb-btn-web" href="${r.website}" target="_blank" rel="noopener">
        ...
        Website
      </a>` : `<button class="nb-btn nb-btn-web" onclick="window._searchNearby('${safeName}')">
...
function showError(el, code) {
  const msgs = {
    ...
    api_fail: { ico:'🔌', title:'Koneksi Gagal',            sub:'Periksa koneksi internet kamu lalu coba lagi.' },
  ...
  const m = msgs[code] || { ico:'⚠️', title:'Gagal Memuat', sub:`Error: ${code}. Coba lagi.` };
  el.innerHTML = `
    <div class="nb-error">
